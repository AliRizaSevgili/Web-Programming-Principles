...
function toCamelCase(name, uppercase) {        
 ...
 for (let index = 0; index < words.length; index++) {
  if (index===0 && !uppercase) {
    camelCaseName += words[index];
  }  else { 

  camelCaseName += words[index].charAt(0).toUpperCase() + words[index].slice(1);
...
const relUse = specialCharacters ? `rel="${rel}"` : `rel=${rel}`;
const hrefUse = specialCharacters ? `href="${href}"` : `href=${href}`;

return `<link ${relUse} ${hrefUse}>`;
}
...
const validFormat1 = /^\d{4}\/\d{2}\/\d{2}$/; // YYYY/MM/DD
const validFormat2 = /^\d{2}\/\d{2}\/\d{4}$/; // DD/MM/YYYY

if (!validFormat1.test(date) && !validFormat2.test(date)) {
  throw new Error(
...
function toDateString(value, format) {
  ...
  try {
    if (!(value instanceof Date) || isNaN(value)) {
      throw new Error('Error: Invalid Date Object');
    }
    ...
    switch (format) {
      ...
      default:
        throw new Error('Error: Invalid date format. Please use "YYYY/MM/DD", "DD/MM/YYYY", or "MM/DD/YYYY".');
    }
  } catch (error) {
...
function normalizeTime(value) {
  const twelveHourFormat = /^(1[0-2]|0?[1-9]):([0-5][0-9]):([0-5][0-9]) (AM|PM)$/;
  const twentyFourHourFormat = /^(2[0-3]|[01]?[0-9]):([0-5][0-9]):([0-5][0-9])$/;

  let match;

  if ((match = twelveHourFormat.exec(value))) {
  ...
  } else if ((match = twentyFourHourFormat.exec(value))) {
...
function formatTimes(...values) {
  if (values.length === 2) {
    let check1 = values[0].split(':');
    let check2 = values[1].split(':');
    if (check1[0] > 23 || check2[0] > 23) {
      return '[]';
    }
    let first = normalizeTime(values[0]);
    let second = normalizeTime(values[1]);
    return `[${first}, ${second}]`;
  } else if (values.length === 3) {
    let first = normalizeTime(values[0]);
    let second = normalizeTime(values[1]);
    let third = normalizeTime(values[2]);
    return `[${first}, ${second}, ${third}]`;
  } else if (values.length === 4) {
    let check1 = values[0].split(':');
    let check2 = values[1].split(':');
    let check3 = values[2].split(':');
    let check4 = values[3].split(':');
    if (check1[0] > 23) {
      var first = '';
    } else {
      first = normalizeTime(values[0]);
    }
    if (check2[0] > 23) {
      var second = '';
    }
  ...
  var times = normalizeTime(values[0]);
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  ...
  if (
    ...
    throw new Error(
      'Invalid argument types. resourceType should be a string, and resourceId should be a string or number.'
    );
  }
  ...
  if (extraData  !== undefined) {
    if (typeof extraData !== 'string' || extraData === '') {
      throw new Error('extraData should be a string.');
    }
